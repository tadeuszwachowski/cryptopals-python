...
'''
IN: wiadomosc
OUT: zestaw macierzy 4x4 stworzonych z wiadomosci
'''
...
'''
S-BOXy niezbedne dla AES (zdefiniowane wczesniej)
'''
aes_sbox = [
...
'''
IN: 1 bajt danych
OUT: 1 bajt danych, zakodowany za pomoca S-BOX
'''
def lookup(byte):
    x = byte >> 4
    y = byte & 15
...
'''
IN: 1 bajt danych
OUT: 1 bajt danych, odkodowany za pomoca S-BOX
'''
...
'''
tzw. key expansion / key schedule
https://en.wikipedia.org/wiki/AES_key_schedule

IN: key: klucz, 
    rounds: liczba rund, ktora wykonamy
OUT: macierz 4x4 stworzona z klucza
'''
...
'''
IN:     rzad z macierzy 4x4
OUT:    rzad przesuniety o 'n' z zawijaniem 
'''
...
'''
bajt ^ bajt z macierzy klucza i macierzy wiadomosci
IN:     block_grid - macierz 4x4 stworzona z wiadomosci
        key_grid - macierz 4x4 stworzona z klucza
OUT:    macierz 4x4
'''
...
'''
IN:     expanded_key - klucz rozszerzony na 128 rund
OUT:    czesc klucza dla danej rundy
'''
...
'''
IN:     key - klucz
        data - dane z wiadomosci
OUT:    zaszyfrowana wiadomosc
'''
def enc(key, data):

    # dodaj padding za pomocą \x00 i podziel na odcinki długosci 16
    ...
    # rozszerz klucz na 128 rund
    expanded_key = expand_key(key, 11)

    # wybierz czesc klucza dla danej rundy
    temp_grids = []
    ...
    # dla ostatniej rundy nie mieszamy kolumn
    temp_grids = []
    ...
    # polacz dane z wszystkich rund w jeden stream
    int_stream = []
...
'''
IN:     key - klucz
        data - zaszyfrowana wiadomosc
OUT     odszyfrowana wiadomosc
'''
def dec(key, data):
    ...
    # odwroc dzialanie ostatniej rundy z szyfrowania
    temp_grids = []
    ...
    for round in range(9, 0, -1):
        ...
        for grid in grids:
            ...
            # mix_columns() 3 razy jest tym samym co uzycie reverse matrix
    ...
    # wez czesc klucza odpowiednia dla danej rundy
    round_key = extract_key_for_round(expanded_key, 0)
...
if __name__ == "__main__":
    with open('07.txt') as f:
        message64 = ''.join(f.read().splitlines())
        ciphertext = base64.b64decode(message64)
